...
public:
    ...
    void update() { _position.x = clamp_value(_position.x, 20.0f, window_width - 20.0f); _position.y = clamp_value(_position.y, 20.0f, window_height - 20.0f); }
...
public:
    ...
    void update(Vector2 target, float delta_time) { const Vector2 d = direction_to(_position, target); _position.x += d.x * _speed * delta_time; _position.y += d.y * _speed * delta_time; }
    bool collides_with(Vector2 position, float radius) const { return CheckCollisionCircles(_position, _radius, position, radius); }
    Vector2 position() const { return _position; }
...
int main()
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (game_over && IsKeyPressed(KEY_R)) reset_game(player, enemies, score, game_over);

        if (!game_over)
        {
            player.update();
            spawn_timer += delta_time;
            if (spawn_timer >= 1.0f) { enemies.push_back(create_enemy()); spawn_timer = 0.0f; }
            for (Bullet& bullet : bullets) bullet.update(delta_time);
            for (Enemy& enemy : enemies)
            {
                enemy.update(player.position(), delta_time);
                if (player.collides_with(enemy.position(), 16.0f)) game_over = true;
            }
            for (size_t enemy_index = 0; enemy_index < enemies.size();)
            {
                bool hit = false;
                for (const Bullet& bullet : bullets) if (bullet.collides_with(player.position(), enemies[enemy_index])) { hit = true; break; }
                if (hit) { enemies.erase(enemies.begin() + enemy_index); ++score; }
                else ++enemy_index;
            }
        }
        ...
        if (game_over) { DrawText("GAME OVER", 500, 300, 40, RAYWHITE); DrawText("Press R to restart", 500, 350, 20, RAYWHITE); }
        EndDrawing();
    }
